...
class DispResNet_scale(nn.Module):

    def __init__(self, alpha=10, beta=0.01):
        super(DispResNet_scale, self).__init__()
        ...
        # add scale prediction
        self.scale_conv1 = nn.Conv2d(conv_planes[6], 1, kernel_size=1, padding=0)
        self.soft_plus = nn.Softplus(beta=1, threshold=20)
        
        # activate for last layer of depth prediction
        self.tanh = nn.Tanh()
    ...
    def forward(self, x):
        ...
        # use tanh to normalize depth
        # disp1 = self.tanh(disp1)
        disp1 = self.alpha * self.predict_disp1(out_iconv1) + self.beta

        scale = self.scale_conv1(out_conv7)
        scale = scale.mean([2,3], keepdim=False)
        scale = scale.view(-1,1,1,1)
        # print(f"disp1: {disp1.shape}, scale: {scale.shape}")
        scale = self.soft_plus(scale)

        disp1 = scale * disp1 + self.beta
